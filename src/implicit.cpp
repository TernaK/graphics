...
implicit_test_t ImplicitNode::hit_test(ray_t& ray, transform_t& transform) {
  implicit_test_t test;
  int idx;
  test.did_hit = hit_test_idx(ray, test.hit, transform, idx);
  if(test.did_hit) {
    test.material = materials[idx];
  }
  return test;
}

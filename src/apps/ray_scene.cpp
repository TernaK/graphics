...
//  cout << tree.get_level();
  ...
  auto mesh = make_shared<Mesh>(MeshType::sphere);
  Transformable transformable;
  transformable.scale = glm::vec3(0.3);
  //make ray
  ray_t ray;
  ...
  for(;;) {
    cv::Mat image = cv::Mat::zeros(300,300,CV_32FC3);
    transformable.position.x = sin(glfwGetTime());
    transformable.position.y = cos(glfwGetTime());
